...
export class Wordle extends SmartContract {
  @state(Field) solutionHash = State<Field>(); //field elements are like unsigned ints with 
  //...a couple extra properties such as you can't modulo them
  @state(UInt32) turnNumber = State<UInt32>();
  @state(Word) lastGuess = State<Word>(); //want to store these vars in the state of the Mina blockchain
...
//these '@' decorators are similar to python decorators. it says 'do this function and 
//...place the output here in place of this chunk of code
  ...
  @method publishHint(solution: Word) {
    ...
    for (let i = 0; i < 5; i++) {
      for (let j = 0; j < 5; j++) {
        ...
        let doesLetterMatch = lastGuess.encodedWord[i].equals(
